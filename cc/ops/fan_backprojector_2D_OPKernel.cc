...
#define CUDA_OPERATOR_KERNEL "FanBackprojection2D"
...
void Fan_Backprojection2D_Kernel_Launcher(const float *sinogram_ptr, float *out, const float *ray_vectors, const int number_of_projections,
                                          const int volume_size_x, const int volume_size_y, const float *volume_spacing, const float *volume_origin,
                                          const int detector_size, const float *detector_spacing, const float *detector_origin,
                                          const float *sid, const float *sdd);

class FanBackprojection2DOp : public OpKernel
{
  public:
    explicit FanBackprojection2DOp(OpKernelConstruction *context) : OpKernel(context)
    {
    }
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        // Grab the volume_shape Tensor. Assuming input Tensor with [batch, number_of_projections, detector_width]
        const Tensor &input_volume_shape = context->input(1);
        auto volume_shape = input_volume_shape.flat_outer_dims<int>();
        ...
        for(int index = 0; index < batch_size; ++index){
          Fan_Backprojection2D_Kernel_Launcher(&input(index,0), &output(index,0), &ray_vectors(index,0), input_shape.dim_size(1),
...
REGISTER_KERNEL_BUILDER(Name(CUDA_OPERATOR_KERNEL).Device(DEVICE_GPU), FanBackprojection2DOp);